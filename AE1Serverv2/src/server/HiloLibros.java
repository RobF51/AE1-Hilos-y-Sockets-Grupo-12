...
public class HiloLibros implements Runnable {
	...
	public void run() {
		...
		Libro libro1 = new Libro("El señor de los Anillos", "84", "Tolkien",
				"30");
		Libro libro2 = new Libro("El Hobbit", "89", "Tolkien", "8.99");
		Libro libro3 = new Libro("Los pilares de la tierra", "87", "Follet",
				"19.99");
		Libro libro4 = new Libro("Dune", "978", "Herbert", "10.99");
		Libro libro5 = new Libro("Don Quijote de La Mancha", "979", "Cervantes",
				"14.99");
		...
		try {
			...
			while (continuar) {
				...
				// almacenamos en la variable texto
				...
				// trim() es un metodo que quita los espacios en blanco del
				// principio
				// y del final
				if (codigos[0].equalsIgnoreCase("0")) {
					// Mandamos la señal de "0" para que el cliente sepa que
					// vamos a cortar
					// la comunicacion
					salida.println("OK");
					System.out.println(
							hilo.getName() + " ha cerrado la comunicacion");
					continuar = false;
				} else {

					// -----------------------------------------------------------------------------
					System.out.println(
							"Se ha realizado una operacion en el servidor por el usuario "
									+ hilo.getName());
					String nombreSol = "";
					...
					switch (opcion) {
						...
						case 2 :
							for (int i = 0; i < listaLibros.size(); i++) {
								...
								if (listaLibros.get(i).getNombre()
										.equalsIgnoreCase(valorCons)) {
									nombreSol = listaLibros.get(i).toString();
								}
						...
						case 3 :
							...
							for (int i = 0; i < listaLibros.size(); i++) {
								...
								if (listaLibros.get(i).getAutor()
										.equalsIgnoreCase(valorCons)) {

									nombreSol += (listaLibros.get(i)
											.toString());
						...
						case 4 :
							...
							nombreSol = listaLibros.get(listaLibros.size() - 1)
									.toString();
